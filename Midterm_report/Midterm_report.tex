...
%\end{theorem}
\begin{defn}
    Let any THC-cromwell matrix G. Consider any Three-row $i$ and its three 1s, $a_{ij}, a_{ik}, a_{il}$, which means that $a_{ij} = a_{ik} = a_{il} = 1$. The \textit{H-deletion Matrix} of the THC-cromwell matrix G is $(n-1)\times(n-1)$ matrix which deleted row $i$ and column $j$, $k$ from the matrix G\\
\end{defn}

\begin{figure*}[h]
    \centerline{\includegraphics[width=0.5\textwidth]{figure/Hdeletion.png}}
    \caption{H deletion of THC-matrix of $\theta$-curve $3_1$.}
    \label{figure_1} 
\end{figure*}
