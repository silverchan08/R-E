...
\begin{columns}[t]
  ...
  \begin{itemize}
    \item \textbf{Cromwell matrix} : an $n\times n$ binary matrix each of whose rows and columns has exactly two 1s
  ...
  \begin{equation*}
    \alpha(G) \leq c(G) + e + b
  \end{equation*}
  where $\alpha(G)$, $c(G)$ are arc index and crossing number of $G$, $e$ is the number of edges, and $b$ is the number of bouquet cut-components, repsectively.
 ...
 % \end{equation*}
  ...
  \begin{block}
    ...
    $\mathbf{Theorem\ 5.}$ Let $H$ be a handcuff graph and $L$ is a constituent link. If $L$ is non-split and alternating, then 
  \begin{equation*}
    \alpha(H) \geq c(L) + 3.
  \end{equation*}
