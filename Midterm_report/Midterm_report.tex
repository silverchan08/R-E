...
\end{defn}
\begin{defn}
    \textit{Handcuff graph} consists of two loops and an edge joining the loops.
\end{defn}

\begin{figure*}[h]
    \centerline{\includegraphics[width=0.5\textwidth]{figure/theta and handcuff graph.png}}
    \caption{Examples of $\theta$-curves and handcuff graphs.}
    \label{figure_1} 
\end{figure*}

In the projection of the handcuff graph(or $\theta$-curve), the section where handcuff graph(or $\theta$-curve) meets itself is named \textit{crossing}. 
...
In the similiar way, a handcuff graph is said to be trivial if 
