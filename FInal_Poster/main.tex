...
\begin{columns}[t]
  ...
  \end{figure}
  $\mathbf{Theorem\ 2.}$ Let $M$ be a THC-cromwell matrice of $\theta$-curve or handcuff graph, and let det*($M$) be the determinant of H-deletion matrix of $M$. Then, \\
		\begin{itemize}
  ...
  $\mathbf{Theorem\ 3.}$ $[$2$]$ For any spatial graph $G$, 
  \begin{equation*}
    \alpha(H) \leq c(H) + e + b
  \end{equation*}
  where $\alpha (G)$, $c(G)$ are arc index, crossing number of $G$, $e$ is the number of edges, and $b$ is the number of bouquet cut-components, respectively.
 ...
 % \end{equation*}
  ...
  \begin{block}
    ...
    $\mathbf{Theorem\ 3.}$ Let $H$ be a handcuff graph and $L$ is a constituent link of $H$. If $L$ is non-split and alternating, then 
  \begin{equation*}
    \alpha(H) \geq c(L) + 3.
  \end{equation*}
    ...
    where $\mu(G)$, $\beta(G)$, and $E$ are the number of connected components, the first Betti number, and the edge set of $G$ respectively. \\
    \textbf{Yamada polynomial} of $G$, $R(G)$, is defined by
    \[ R(G)(x) = h(G)(-1, -x-2-x^{-1}). \]
    
    $\mathbf{Theorem\ 4.}$ $[$1$]$ Let $S_T$ be a simple closure of stacked tangle of a $\theta$-curve or handcuff graph, and $c, n$ be the number of caps and crossings in $S_T$, respectively. Then,
     \[ \mathrm{spr}(R(S_T)) \leq c+2n \]
     where $\mathrm{spr}(f)$ denotes the difference between the maximal and minimal degrees of $f$. 

    $\mathbf{Theorem\ 5.}$ Let $T$ be a $\theta$-curve or handcuff graph. Then,
...
{\small
    \begin{enumerate}
      \item[1] Yoonsang Lee. (2023). A Study on Arc Index of Theta-Curves. Korea Science Academy of KAIST
      \item[2] Minjung Lee, Sungjong No, and Seungsang Oh. (2018). Arc index of spatial graphs. \textit{Journal of Graph Theory, 90}(3), 406–415.
      \item[3] Moriuchi, H. (2019). A table of $\theta$-curves and handcuff graphs with up to seven crossings. \textit{Advanced Studies in Pure Mathematics}, 281–290.
