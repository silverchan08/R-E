...
\begin{columns}[t]
  ...
  \begin{itemize}
    \item \textbf{Stacked tangle} of an theta-curve or handcuff graph is a configuration of stacked disks with arcs and caps.
    \item \textbf{Arcs} are segments properly embedded in each disk, with endpoints on the boundary.
    \item \textbf{Caps} are segments from the arc in one disk to the arc in another disk.
    \item A \textbf{Simple closure} of stacked tangle is the planar projection of the stacked tangle without any nested caps.
  \end{itemize}
