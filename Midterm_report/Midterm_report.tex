...
\end{theorem}
\begin{defn}
    Let any THC matrix G. Consider any Three-row $i$ and its three 1s, $a_{ij}, a_{ik}, a_{il}$, which means that $a_{ij} = a_{ik} = a_{il} = 1$. The \textit{H-deletion Matrix} is $(n-1)\times(n-1)$ matrix which deleted row $i$ and column $j$, $k$ from the Matrix G\\
\end{defn}
