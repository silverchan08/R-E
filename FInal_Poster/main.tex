...
\begin{columns}[t]
 ...
 % \end{equation*}
  ...
  \end{equation*}
    ...
    \includegraphics[width=\textwidth]{figure/stackeds.png}
    
    $\mathbf{Theorem\ 7.}$ Let $S_G$ be a simple closure of stacked tangle of a $\theta$-curve or handcuff graph $G$. For $c$ and $n$ be the number of caps and crossings in $S_T$, respectively. Then,
     \[ \mathrm{spr}(R(S_T)) \leq c+2n \]
     where $\mathrm{spr}(f)$ denotes the difference between the maximal and minimal degrees of $f$.

    $\mathbf{Theorem\ 8.}$ Let $G$ be a $\theta$-curve or handcuff graph. Then,
