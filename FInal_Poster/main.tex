...
\begin{columns}[t]
  ...
  \begin{equation*}
    \alpha(G) \leq c(G) + e + b
  \end{equation*}
  where $\alpha(G)$, $c(G)$ are arc index and crossing number of $G$, and $e, b$ are the number of edges and bouquet cut-components, repsectively.
 ...
 % \end{equation*}
  ...
  \end{equation*}
    ...
    where $\mu(G)$, $\beta(G)$, $E$ are the number of connected components, the first Betti number, and the edge set of $G$ respectively. \\
    \textbf{Yamada polynomial} of $G$, $R(G)$, is defined by
    \[ R(G)(x) = h(G)(-1, -x-2-x^{-1}). \]
    
    $\mathbf{Theorem\ 7.}$ Let $S_T$ be a simple closure of stacked tangle of a $\theta$-curve or handcuff graph $T$, and $c, n$ be the number of caps and crossings in $S_T$, respectively. Then,
