...
\begin{frame}{Lower Bounds from Constituent Knots/Links}
	\begin{thm}
...
\begin{frame}{Lower Bounds from Constituent Knots/Links}
	...
	\end{itemize}
	\[
		\alpha(K_i) \le |S_i| + |S_{i+1}| \implies \sum_{i=1}^3 \alpha(K_i) \le 2\sum_{i=1}^3|S_i| = 2 \alpha(T)
	\]
	\hfill\qed
\end{frame}
...
\begin{frame}{Lower Bounds from Constituent Knots/Links}
	...
	\mypf
	\begin{itemize}
		\item We know $\alpha(H) \ge \alpha(L) + 1$.
		\item By the previous theorem, $\alpha(L) = c(L) + 2$.
		\item Thus, $\alpha(H) \ge c(L) + 3$.
	\end{itemize}
	\hfill\qed
\end{frame}
...
\begin{frame}{Lower Bounds from Yamada Polynomial}
	...
	\mypf
	\begin{itemize}
		\item Let \myem{$c_s, c_{ss}$} be the number of \myem{simlpe caps} and \myem{semi-simple caps}, repectively.
...
\begin{frame}{Proof of Theorem}
	...
	When $c_s + c_{ss} = 0$,
		\begin{itemize}
			\item $S_T$ has no simple disks.
			\item $S_T$ is equivalent to the result of applying $\myem{Y14}$ to \raisebox{-3pt}{\includegraphics[height=12pt]{y6}}.
			\item Thus, $R(S_T) = -x^{\pm 3} \left[ -x^2 - x - 2 - x^{-1} - x^{-2} \right] \implies 5 \le c + n$.
		\end{itemize}
	When $n = 0$,
		\begin{itemize}
			\item $S_T$ is equivalent to $\raisebox{-1.5pt}{\includegraphics[height=8pt]{y7}}\cup\bigcirc\cup\cdots\cup\bigcirc$.
			\item Thus, $R(S_T) = 0 \implies 0 < 2 \le c + n$.
		\end{itemize}
...
\begin{frame}{Proof of Theorem}
	...
	$$\includegraphics[width=.25\linewidth]{induction_disks.png}$$
\end{frame}
...
\begin{frame}{Proof of Theorem}
	...
	\end{enumerate}
		...
		\begin{itemize}
			...
			$$\includegraphics[width=.4\linewidth]{three_cases.png}$$
...
\begin{frame}{Proof of Theorem}
	...
	\begin{align*}
		\max\deg_xR(S_T) - \min\deg_xR(S_T) & \le 2n + \alpha(T) \le \alpha(T)^2 - 4\alpha(T) + 8 \\
		2 + \sqrt{\max\deg_xR(S_T) - \min\deg_xR(S_T) - 4} & \le \alpha(T) \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \quad \qed
	\end{align*}
\end{frame}