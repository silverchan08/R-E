...
\begin{defn}
    ...
    The 1s contained in both ith row and jth, lth columns are called the \textit{edge-ones}, and the 1 contained in both ith row and kth column is called the \textit{middle-one}.\\
\end{defn}
...
\begin{enumerate}
    \item \textbf{When $n = 0$}\\
    Actually, the 1s in same column represents the vertical line of the grid diagram, and the 1s in same row represents the horizontal line of the grid diagram.
    ...
    As we consider the non-deleated row's middle-one, it is not deleated since $n=0$.
    ...
    \item \textbf{When $n = 1$ (middle vertex)}\\
    The grid diagram represents the graph consisted of a vertex and other vertice with an edges connecting a vertex with the other vertex.
    We'll call in as the \textit{Line-shape}
    \item \textbf{When $n = 1$ (end vertex)}\\
    Line-shape is given.
    \item \textbf{When $n = 2$ (two end vertices)}\\
    T-shape is given.
    \item \textbf{When $n = 2$ (middle and end vertices)}\\
    Line-shape is given.
\end{enumerate}
