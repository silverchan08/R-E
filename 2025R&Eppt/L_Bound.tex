...
\begin{frame}{Lower Bound from Constituent Knots/Links}
	...
	\begin{corollary}
		Let $H$ be a handcuff graph, and $L$ be a constituent link of $T$. If $L$ is alternating and non-split,
		\[
			\alpha(H) = c(L) + 3.
		\]
	\end{corollary}
...
\begin{frame}{Lower Bound from Yamada Polynomial}
	...
	\mypf
	\begin{itemize}
		\item Let \myem{$c_s$} and \myem{$c_{ss}$} be the number of \myem{simlpe caps} and \myem{semi-simple caps}, repectively.
...
\begin{frame}
	\begin{prop}
		...
		corresponding to minimal arc presentation of $G$.
		Then
		\[
	...
	\begin{itemize}
		\item $S_G$ is a reduced simple closure of stacked tangle corresponding to minimal arc presentation.
		\item The number of caps $c$ in $S_G$ is exactly arc index of $G$, $\alpha(G)$.
	\end{itemize}
\end{frame}
...
\begin{frame}{Proof of Theorem}
	...
	Let $S_G$ be a reduce simple closure of stacked tangle of a $\theta$-curve or handcuff graph $G$
	corresponding to minimal arc presentation of $G$.
	...
	Consider the maximum number of crossings in $S_G$, and use the previous theorem.
\end{frame}